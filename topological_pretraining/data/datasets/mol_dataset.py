...
class MolDataset:
	...
	def __init__(
		...
		featurizer_kwargs: dict | None = None,
		extra_transform_kwargs: dict | None = None,
		verbose: bool = False,
		...
		extra_transform_kwargs = extra_transform_kwargs or {}
	...
	def set_extra_transform(self, kwargs):
		...
		for name, vals in kwargs.items():
			...
			if name == 'select_k_best' and 'score_func' not in vals:
				raise ValueError(
					'score_func must be provided for SelectKBest.\
		...
		print(
			f'Extra transforms set.\nPipeline: {self._extra_transform}'
	...
	def reset(self, train_idx: np.ndarray, test_idx: np.ndarray | None = None) -> None:
		"""
		...
		if isinstance(self.X, np.ndarray):
			if np.any(np.isnan(self.X[train_idx])) or np.any(
				np.isnan(self.X[test_idx])
			):
				...
				print(
					f'NaN values found in X; number of cols = {num_cols}; number of rows = {num_rows}; total NaNs = {num_nans}.'
				)
				print(
					'Fitting imputer on train data for replacing NaN values with mean...'
				)
		...
		if 'select_k_best' in self._extra_transform.named_steps:
			...
			print(
				'Setting k for SelectKBest to number of train samples - 1...'
			) if self.verbose else None
	...
	@property
	def test_y(self):
		if self.test_idx is None or self.y is None:
			return None
		else:
