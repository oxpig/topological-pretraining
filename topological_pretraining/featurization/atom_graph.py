...
class AtomGraph(BaseGraph):
	...
	**kwargs : dict
	...
	def __init__(self, node_types: dict = None, edge_types: dict = None, **kwargs):
		super().__init__(node_types=node_types, edge_types=edge_types)
...
class AtomFeatureGraph(BaseGraph):
	...
	def __init__(self, verbose: bool = False, global_token: bool = False):
		self.global_token = global_token
		self.verbose = verbose
		self.atom_encoder = OneHotEncoder(categories=self.permitted_atoms)
	...
	def get_atom_features(self, atom: Chem.Atom):
		...
		n_heavy_neighbors = self.n_heavy_neighbors.transform(atom.GetDegree())
		...
		hydrogen_neighbors = self.hydrogen_neighbors.transform(atom.GetTotalNumHs())
		return np.hstack(
			[
				atom_type,
				n_heavy_neighbors,
	...
	def get_nodes(self, mol: Chem.Mol):
		...
		"""
		x = torch.full(
			(mol.GetNumAtoms(), self.n_features), fill_value=-1, dtype=torch.float32
		)
		for atom in mol.GetAtoms():
