...
from topological_pretraining.data.mol import MolDesc, MorganGenerator, SortAndSlice
...
class SNS(BaseFeaturizer):
	...
	def __init__(self, identifiers: dict = None, encoder: dict = None, **kwargs):
		super().__init__(**kwargs)
		self.transform.identifiers = identifiers or {}
		self.transform.encoder = encoder or {}
	...
	def fit(self, mols: list[Chem.Mol], y: np.ndarray | None = None) -> None:
		"""
	...
	def preprocess(self, mols: list[Chem.Mol]) -> list[np.ndarray]:
		...
		"""
		return [
			self.transform.generator.environments(mol) if mol is not None else mol
			for mol in mols
		]
