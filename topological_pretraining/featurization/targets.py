...
class Targets(dict):
	...
	def __init__(
		self, targets: dict[str, dict[str, str]] = None, targets_path: str = None
	):
	...
	def transform(self, mol: Chem.Mol, graph: pyg.data.Data):
		...
		for target_name in self:
			...
			dtype = torch.float32 if prediction_head == 'regression' else torch.long
			X = mol if input_type == 'molecule' else graph
